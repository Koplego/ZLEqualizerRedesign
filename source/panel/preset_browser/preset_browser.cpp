...
namespace zlpanel {
    namespace {
        juce::File getPresetsDirectory() {
            return juce::File::getSpecialLocation(juce::File::userApplicationDataDirectory)
                .getChildFile("ZL Audio").getChildFile(JucePlugin_Name).getChildFile("Presets");
        }

        juce::File getLegacyPresetsDirectory() {
            return juce::File::getSpecialLocation(juce::File::userApplicationDataDirectory)
                .getChildFile("Audio").getChildFile("Presets")
                .getChildFile(JucePlugin_Manufacturer).getChildFile(JucePlugin_Name).getChildFile("Presets");
        }
        ...
        juce::Result ensureDirectoryExists(const juce::File& directory) {
            ...
            if (parent == directory) return juce::Result::fail("Could not find a parent directory");
            if (const auto result = ensureDirectoryExists(parent); result.failed()) return result;
        ...
        juce::Result migrateLegacyPresets(const juce::File& destination) {
            ...
            if (!legacy.isDirectory() || destination.exists()) return juce::Result::ok();
            ...
            if (!temporary.moveFileTo(destination)) {
                [[maybe_unused]] const auto flag = temporary.deleteRecursively();
                return destination.isDirectory() ? juce::Result::ok()
    ...
    PresetBrowser::PresetBrowser(PluginProcessor& processor, zlgui::UIBase& base) :
        processor_(processor), base_(base), presets_directory_(getPresetsDirectory()), background_(base),
        delete_drawable_(juce::Drawable::createFromImageData(BinaryData::trash_svg, BinaryData::trash_svgSize)),
        close_drawable_(juce::Drawable::createFromImageData(BinaryData::close_svg, BinaryData::close_svgSize)),
        folder_open_drawable_(juce::Drawable::createFromImageData(BinaryData::folder_open_svg, BinaryData::folder_open_svgSize)),
        ...
        group_label_({}, "Groups"), preset_label_({}, "Presets"), search_editor_(base),
        group_name_editor_(base), preset_name_editor_(base), group_list_(base), preset_list_(base),
        ...
        group_name_editor_.onReturnKey = [this]() { createGroup(); group_name_editor_.giveAwayKeyboardFocus(); };
        preset_name_editor_.onReturnKey = [this]() { savePreset(); preset_name_editor_.giveAwayKeyboardFocus(); };
        ...
        close_button_.getButton().onClick = [this]() { base_.setPanelProperty(zlgui::PanelSettingIdx::kPresetBrowser, 0.f); };
        folder_open_button_.getButton().onClick = [this]() { revealFolder(); };
    ...
    void PresetBrowser::resized() {
        ...
        const auto padding = juce::roundToInt(font * .78f);
        const auto button_size = juce::roundToInt(font * 1.62f);
        const auto row_height = juce::roundToInt(font * 1.55f);
        const auto header_height = juce::roundToInt(font * 2.05f);
        const auto footer_height = juce::roundToInt(font * 2.05f);
        const auto list_top_margin = juce::roundToInt(font * .28f);
        const auto column_text_inset = preset_list_layout::columnTextInset(font);

        auto bounds = getLocalBounds().reduced(padding);
        auto header = bounds.removeFromTop(header_height);
        auto footer = bounds.removeFromBottom(footer_height);
        footer.removeFromTop(juce::jmax(2, padding / 3));

        const auto minimum_group_width = juce::roundToInt(font * 9.2f);
        const auto group_width = juce::jlimit(minimum_group_width,
                                              juce::jmax(minimum_group_width, bounds.getWidth() / 2),
                                              juce::roundToInt(static_cast<float>(bounds.getWidth()) * .26f));

        auto group_header = header.removeFromLeft(group_width);
        header.removeFromLeft(padding);
        delete_group_button_.setBounds(group_header.removeFromRight(button_size).withSizeKeepingCentre(button_size, button_size));
        group_header.removeFromRight(padding / 3);
        folder_open_button_.setBounds(group_header.removeFromRight(button_size).withSizeKeepingCentre(button_size, button_size));
        folder_open_button_.getButton().setEdgeIndent(static_cast<int>(std::round(font * .18f)));
        group_label_.setBounds(group_header);

        close_button_.setBounds(header.removeFromRight(button_size).withSizeKeepingCentre(button_size, button_size));
        header.removeFromRight(padding / 3);
        delete_preset_button_.setBounds(header.removeFromRight(button_size).withSizeKeepingCentre(button_size, button_size));
        header.removeFromRight(padding / 2);
        const auto preset_label_width = juce::jmin(header.getWidth() / 3, juce::roundToInt(font * 6.2f));
        preset_label_.setBounds(header.removeFromLeft(preset_label_width));
        search_editor_.setBounds(header.reduced(0, juce::jmax(1, padding / 5)));
        ...
        background_.setSurfaceBounds({search_editor_.getBounds(), group_list_.getBounds(), preset_list_.getBounds(),
                                      group_name_editor_.getBounds(), preset_name_editor_.getBounds()});

        const auto editor_top_inset = juce::roundToInt(font * .11f);
    ...
    void PresetBrowser::visibilityChanged() {
        ...
        else warning_overlay_.hide();
    }

    void PresetBrowser::refresh() {
        if (!ensurePresetDirectory()) return;
        refreshGroups();
        refreshPresetCache();
        refreshPresets();
    }
    ...
    int PresetBrowser::getIdealWidth() const { return juce::roundToInt(base_.getFontSize() * 42.f); }
    int PresetBrowser::getIdealHeight() const { return juce::roundToInt(base_.getFontSize() * 25.5f); }

    bool PresetBrowser::ensurePresetDirectory() {
        if (const auto result = migrateLegacyPresets(presets_directory_); result.failed())
            showError(getDirectoryCreationError("Could not migrate the existing presets.", getLegacyPresetsDirectory(), result));
        if (const auto result = ensureDirectoryExists(presets_directory_); result.failed()) {
            showError(getDirectoryCreationError("Could not create the preset directory.", presets_directory_, result));
            return false;
        }
        ...
        if (const auto result = ensureDirectoryExists(user_directory); result.failed()) {
            showError(getDirectoryCreationError("Could not create the User group.", user_directory, result));
            return false;
        }
        return true;
    }
    ...
    void PresetBrowser::refreshGroups() {
        auto directories = presets_directory_.findChildFiles(juce::File::findDirectories | juce::File::ignoreHiddenFiles, false);
        std::sort(directories.begin(), directories.end(), [](const auto& lhs, const auto& rhs) {
            if (lhs == rhs) return false;
            if (lhs.getFileName() == kDefaultGroup) return true;
            if (rhs.getFileName() == kDefaultGroup) return false;
            return lhs.getFileName().compareNatural(rhs.getFileName()) < 0;
        });
        groups_.clear();
        groups_.add(kAllPresetsGroup);
        for (const auto& directory : directories) groups_.add(directory.getFileName());
        if (!groups_.contains(selected_group_)) selected_group_ = kDefaultGroup;
        group_list_.setGroups(groups_, selected_group_);
        updateDeleteButtonStates();
    }

    void PresetBrowser::refreshPresetCache() {
        const auto files = presets_directory_.findChildFiles(juce::File::findFiles | juce::File::ignoreHiddenFiles,
                                                             true, "*" + juce::String{kPresetExtension});
        all_presets_.clear();
        all_presets_.reserve(static_cast<size_t>(files.size()));
        for (const auto& file : files)
            all_presets_.push_back({file.getFileNameWithoutExtension(), file.getParentDirectory().getFileName(), file});
    ...
    void PresetBrowser::refreshPresets() {
        ...
        for (const auto& preset : all_presets_) {
            if (selected_group_ != kAllPresetsGroup && preset.group != selected_group_) continue;
            if (search.isNotEmpty() && !preset.name.toLowerCase().contains(search) && !preset.group.toLowerCase().contains(search)) continue;
            presets_.push_back(preset);
        }
        if (getSelectedPreset() == nullptr) selected_preset_file_ = juce::File{};
    ...
    void PresetBrowser::createGroup() {
        ...
        if (!isVisibleFileName(legal_name) || legal_name.equalsIgnoreCase(kAllPresetsGroup)) {
            showError("Enter a valid group name."); return;
        }
        ...
        if (const auto result = ensureDirectoryExists(directory); result.failed()) {
            showError(getDirectoryCreationError("Could not create group \"" + legal_name + "\".", directory, result)); return;
        }
    ...
    void PresetBrowser::deleteSelectedGroup() {
        if (selected_group_ == kAllPresetsGroup || selected_group_ == kDefaultGroup) {
            showError("This group cannot be deleted."); return;
        }
        const auto directory = presets_directory_.getChildFile(selected_group_);
        if (!directory.isDirectory()) {
            showError("The group is no longer available."); refresh(); return;
        }
        const auto group = selected_group_;
        const auto preset_count = static_cast<int>(std::count_if(all_presets_.begin(), all_presets_.end(),
                                                                 [&group](const auto& preset) { return preset.group == group; }));
        const auto count_text = juce::String{preset_count} + (preset_count == 1 ? " preset" : " presets");
        warning_overlay_.show("Delete \"" + group + "\" and all of its contents (" + count_text + ")?",
                              "Delete", true, [this, directory, group]() {
            if (!directory.deleteRecursively()) { showError("Could not delete group \"" + group + "\"."); return; }
            selected_group_ = kDefaultGroup; selected_preset_file_ = juce::File{}; preset_name_editor_.clear(); refresh();
        });
    }
    ...
    void PresetBrowser::savePreset() {
        ...
        if (legal_name.endsWithIgnoreCase(kPresetExtension)) legal_name = legal_name.dropLastCharacters(juce::String{kPresetExtension}.length());
        if (!isVisibleFileName(legal_name)) { showError("Enter a preset name."); return; }
        const auto group = getWritableGroup();
        const auto file = presets_directory_.getChildFile(group).getChildFile(legal_name + kPresetExtension);
        if (!file.existsAsFile()) { writePreset(file); return; }
    ...
    void PresetBrowser::writePreset(const juce::File& file) {
        ...
        if (const auto result = PresetJson::write(file, state); result.failed()) {
            showError("Could not save preset \"" + file.getFileNameWithoutExtension() + "\"."); return;
        }
    ...
    void PresetBrowser::loadPreset(const juce::File& file) {
        const auto iterator = std::find_if(presets_.begin(), presets_.end(), [&file](const auto& preset) { return preset.file == file; });
        if (iterator == presets_.end()) { showError("The preset is no longer available."); return; }
        juce::MemoryBlock state;
        if (const auto result = PresetJson::read(file, state); result.failed()) {
            showError("Preset \"" + iterator->name + "\" is invalid."); return;
        }
    ...
    void PresetBrowser::deleteSelectedPreset() {
        ...
        if (selected == nullptr) { showError("Select a preset to delete."); return; }
        const auto preset = *selected;
        warning_overlay_.show("Delete \"" + preset.name + "\" from \"" + preset.group + "\"?",
                              "Delete", true, [this, preset]() {
            if (!preset.file.deleteFile()) { showError("Could not delete \"" + preset.name + "\"."); return; }
            selected_preset_file_ = juce::File{}; preset_name_editor_.clear(); refreshPresetCache(); refreshPresets();
        });
    }

    void PresetBrowser::selectGroup(const juce::String& group) {
        if (!groups_.contains(group) || selected_group_ == group) return;
    ...
    void PresetBrowser::configureEditor(RoundedTextEditor& editor, const juce::String& placeholder) {
        editor.setTextToShowWhenEmpty(placeholder, base_.getTextInactiveColour().withAlpha(.62f));
    ...
    void PresetBrowser::applyColours() {
        for (auto* label : {&group_label_, &preset_label_})
            label->setColour(juce::Label::textColourId, base_.getTextColour().withAlpha(.70f));
        for (auto* editor : {&search_editor_, &group_name_editor_, &preset_name_editor_}) {
            editor->setColour(juce::TextEditor::textColourId, base_.getTextColour().withAlpha(.88f));
            editor->setColour(juce::TextEditor::highlightColourId, base_.getTextColour().withAlpha(.16f));
    ...
    void PresetBrowser::showError(const juce::String& message) { warning_overlay_.showMessage(message); }
    ...
    void PresetBrowser::updateDeleteButtonStates() {
        ...
        delete_group_button_.setAlpha(can_delete_group ? .82f : .20f);
        ...
        delete_preset_button_.setAlpha(can_delete_preset ? .82f : .20f);
    ...
    void PresetBrowser::valueTreePropertyChanged(juce::ValueTree&, const juce::Identifier& property) {
        if (!base_.isPanelIdentifier(zlgui::PanelSettingIdx::kPresetBrowser, property)) return;
        const auto should_be_visible = static_cast<float>(base_.getPanelProperty(zlgui::PanelSettingIdx::kPresetBrowser)) > .5f;
        setVisible(should_be_visible);
        if (should_be_visible) toFront(false);
    }
    ...
    juce::String PresetBrowser::getWritableGroup() const {
        if (selected_group_ != kAllPresetsGroup) return selected_group_;
        if (const auto* selected = getSelectedPreset(); selected != nullptr) return selected->group;
        return kDefaultGroup;
    }

    void PresetBrowser::revealFolder() {
        if (presets_directory_.isDirectory()) presets_directory_.revealToUser();
        else showError("Could not open the preset directory.");
    }
}
