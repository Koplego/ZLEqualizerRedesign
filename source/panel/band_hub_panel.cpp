...
namespace zlpanel {
    BandHubPanel::BandHubPanel(PluginProcessor& p, zlgui::UIBase& base,
        ...
        dynamic_button_.getButton().onClick = [this]() {
            if (const auto band = base_.getSelectedBand(); band < zlp::kBandNum) {
                ...
                updateValue(p_ref_.parameters_.getParameter(
                    zlp::PDynamicON::kID + std::to_string(band)), dynamic_on_ ? 1.f : 0.f);
    ...
    void BandHubPanel::paint(juce::Graphics& g) {
        ...
        // The hub stays clear glass. Nearby node lights enter through its surface,
        // clipped to the moving lens outline so no rectangular colour field appears.
        {
            juce::Path lens;
            lens.addRoundedRectangle(surface, radius);
            juce::Graphics::ScopedSaveState state(g);
            g.reduceClipRegion(lens);
            for (const auto& light : ambient_sources_) {
